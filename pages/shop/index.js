...
const ShopPage = () => {
    ...
    const sortMyProducts = (products, sortBy) => {
        if(sortBy === "featured"){
    ...
    useEffect(() => {
        ...
        if(choosenFilters.length > 0){
            ...
            console.log("choosenFilters----useEffect", choosenFilters)   

            let selectedFiltersArray = choosenFilters.map((filter) => {
                return filter.name
            })
            ...
            if(selectedFiltersArray){
                const filteredProducts = products.filter((product) => selectedFiltersArray.includes(product.mainCategory))
    ...
    const handleSubItemChange = (filterLabel, subItemId, e) => {
        // Stop event propagation to prevent bubbling up to parent elements
        ...
        setChoosenFilters(selectedFilters[0])
    }


    const handleMainFilterChange = (filterLabel) => {
        let newFilterDataRaw = filterDataRaw.map((filter) => {
            if(filter.label === filterLabel){
                filter.selected = !filter.selected
            }
            return filter
        })
        setFilterDataRaw(newFilterDataRaw)
    }
                        ...
                        <div className={styles.filterInner}>
                            ...
                            <div className={styles.mainFilters}>
                                {filterDataRaw.map((filter) => (
                                    <div key={filter.label} className={styles.filterItem} onClick={() => handleMainFilterChange(filter.label)}>
                                        <span>{filter.label}</span>
                                        {filter?.selected && <div className={styles.subFilters}>
                                            {filter.subItems.map((subItem) => (
                                                <div key={subItem.id} className={styles.subItem}>
                                                    <input type="checkbox" checked={subItem.selected} onChange={(e) => handleSubItemChange(filter.label, subItem.id, e)} />
                        ...
                        {filteredProducts.map((product) => (
                            <div key={product.id} className={styles.productCard}>
                                ...
                                <div className={styles.productDetails}>
                                    ...
                                    <span className={styles.price}>MRP: {product.mainCategory}</span>
                                </div>
                            </div>
                        ))}
                    </div>
